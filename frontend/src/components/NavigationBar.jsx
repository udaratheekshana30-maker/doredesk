...
export default function NavigationBar() {
    ...
    const allNavItems = [
        { to: '/dashboard', icon: HiOutlineHome, label: 'Dashboard', roles: ['warden'] },
        { to: '/room-management', icon: HiOutlineBuildingOffice2, label: 'Floor & Room', roles: ['warden', 'roombookingadmin'] },
        { to: '/student-applications', icon: HiOutlineUserGroup, label: 'Allocations', roles: ['warden', 'roombookingadmin'] },
        { to: '/profiles', icon: HiOutlineUser, label: 'Profiles', roles: ['warden'] },
        { to: '/warden/laundry', icon: MdOutlineLocalLaundryService, label: 'Laundry', roles: ['warden', 'laundryadmin'] },
        { to: '/warden/resources', icon: HiOutlineUserGroup, label: 'Facilities', roles: ['warden', 'resourceadmin'] },
        { to: '/records', icon: HiOutlineDocumentChartBar, label: 'Records', roles: ['warden'] },
        { to: '/resources', icon: HiOutlineCube, label: 'Inventory', roles: ['warden'] },
        { to: '/notices', icon: HiOutlineMegaphone, label: 'Notices', roles: ['warden'] },
        { to: '/warden/settings', icon: HiOutlineCog6Tooth, label: 'Settings', roles: ['warden', 'laundryadmin', 'resourceadmin', 'roombookingadmin'] }
    ]

    const updatedNavItems = allNavItems.filter(item => 
        ...
        <header className={`nav-bar ${isScrolled ? 'scrolled' : ''} ${!isVisible ? 'hidden-nav' : ''}`}>
            <div className="nav-container">
                <div className="nav-logo-section">
                    <img src={logo} alt="DormDesk Logo" className="nav-logo-img" />
                    <div className="flex flex-col">
                        <div className="nav-title text-[15px] sm:text-[18px]">DormDesk <span className="text-[#FAB95B]">UNI</span></div>
