...
function isValidDormEmail(email) {
  return /^[a-zA-Z]{2}\d{8}@my\.dorm\.lk$/i.test((email || "").trim());
}
...
function validateApplicationData(payload) {
  ...
  if (payload.studentEmail && !isValidDormEmail(payload.studentEmail)) {
    return "studentEmail must match a valid DormDesk format (e.g. itxxxxxxxx@my.dorm.lk or bmxxxxxxxx@my.dorm.lk).";
  }
...
// Layer 3: DormDesk email submitted in the form body (extraEmail param)
async function findMyApplication(userId, extraEmail) {
  ...
  // Layer 3 — by DormDesk email from form body (studentEmail field)
...
async function updateMyApplication(req, res) {
  try {
    ...
    // Use robust 3-layer lookup — includes the DormDesk email from the form body
