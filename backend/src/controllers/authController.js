    ...
    try {
        ...
        if (!studentEmailRegex.test(email)) {
            return res.status(400).json({ 
                success: false, 
                message: 'Invalid email: Must be a valid DormDesk student email (e.g., ITXXXXXXXX@my.dorm.lk, BMXXXXXXXX@my.dorm.lk or SAXXXXXXXX@my.dorm.lk)' 
            });
        }
        ...
        const emailResult = await sendEmail({
            email,
            subject: 'DormDesk - Email Verification Code',
            message: `
                <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                    <h2 style="color: #4f46e5;">Email Verification</h2>
                    <p>Hello,</p>
                    <p>Your verification code for DormDesk Management System signup is:</p>
...
const register = async (req, res) => {
    try {
        ...
        // Check email format: (Any Student ID prefix, e.g., IT, BM)XXXXXXXX@my.dorm.lk
        const studentEmailRegex = /^[a-zA-Z]{2}\d{8}@my\.dorm\.lk$/i;
        if (!studentEmailRegex.test(email)) {
            return res.status(400).json({ 
                success: false, 
                message: 'Registration failed: Student email must be a valid DormDesk student address (e.g., ITXXXXXXXX@my.dorm.lk, BMXXXXXXXX@my.dorm.lk or SAXXXXXXXX@my.dorm.lk)' 
            });
        }
